...
- **LeetCode-style theme, light + dark.** Colors are CSS variables in `src/index.css` (`:root` = light `#f7f8fa`/`#fff`; `.dark` = `#1a1a1a`/`#282828`), mapped to Tailwind tokens via `rgb(var(--x) / <alpha-value>)` in `tailwind.config.js` (`darkMode: "class"`). So `bg-surface`/`text-ink`/`border-edge`/etc. flip automatically — **use the tokens, never hardcode hex** (the only constants are the accents `grind` green / `gold` orange / `danger` red, identical in both modes, and the per-user `USER_COLORS` for dots/bars/chart lines). Theme = a `dark` class on `<html>`, set by `src/lib/theme.ts` (system default + persisted override), applied in `main.tsx` before paint; `ThemeToggle` flips it. Font is the system UI sans stack; `font-mono` (system mono) is reserved for numeric values/codes. Keep it simple — no gradients or all-caps mono labels.
